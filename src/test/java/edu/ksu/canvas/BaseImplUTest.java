...
public class BaseImplUTest extends CanvasTestBase {
    ...
    public void readAsCanvasUserIsNotObviouslyNotThreadsafe() throws IOException {
        fakeRestClient.addSuccessResponse(canvasReader.buildTestUrl(TEST_OBJECT_ID_1) + "?as_user_id=user1", "TestModels/TestModel.json");
        fakeRestClient.addSuccessResponse(canvasReader.buildTestUrl(TEST_OBJECT_ID_1), "TestModels/TestModel.json");
        fakeRestClient.addSuccessResponse(canvasReader.buildTestUrl(TEST_OBJECT_ID_1)+ "?as_user_id=user2", "TestModels/TestModel.json");
        ...
        verify(restClientSpy).sendApiGet(eq(SOME_OAUTH_TOKEN), eq(TEST_OBJECT_ID_1 + "?as_user_id=user1"), anyInt(), anyInt());
        verify(restClientSpy).sendApiGet(eq(SOME_OAUTH_TOKEN), eq(TEST_OBJECT_ID_1 + "?as_user_id=user2"), anyInt(), anyInt());
    }
